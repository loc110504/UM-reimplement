...
from model.smp_deeplab import DeepLabV3PlusSMPPerturb
...
def main():
    ...
    # Load cấu hình từ file YAML (bao gồm các thông số như learning rate, batch_size, crop_size, etc.)
    ...
    # Tăng tốc độ xử lý với GPU
    cudnn.benchmark = True
    cudnn.enabled = True

    # --- KHỞI TẠO MÔ HÌNH với DeepLabV3PlusSMPPerturb ---
    model = DeepLabV3PlusSMPPerturb(
        encoder_name=cfg['backbone'],  # Ví dụ: 'resnet101'
        encoder_weights=cfg.get('encoder_weights', 'imagenet'),
        in_channels=cfg.get('in_channels', 3),
        classes=cfg['nclass']
    )

    # --- Optimizer: chia theo 2 nhóm tham số: encoder và các phần còn lại ---
    optimizer = SGD([
                    {'params': model.model.encoder.parameters(), 'lr': cfg['lr']},
    ...
    # --- Loss function cho labeled data ---
    ...
    for epoch in range(start_epoch, cfg['epochs']):
        ...
        # Lấy 1 batch từ labeled và 2 batch từ unlabeled
        # Batch thứ nhất của unlabeled dùng cho loss chính,
        # Batch thứ hai dùng cho cutmix.
        ...
        for i, ((img_x, mask_x),
            ...
            # Dự đoán trên ảnh unlabeled mix để lấy pseudo-label và độ tin cậy
            with torch.no_grad():
                model.eval()
            ...
            # --- Forward pass với need_fp=True để nhận về 2 đầu ra ---
            # Gộp dữ liệu có nhãn và không nhãn lại theo chiều batch
            preds, preds_fp = model(torch.cat((img_x, img_u_w)), need_fp=True)
            pred_x, pred_u_w = preds.split([num_lb, num_ulb])
            pred_u_w_fp = preds_fp[num_lb:]  # Lấy nhánh perturbed cho unlabeled

            # Dự đoán từ 2 ảnh augmentation của unlabeled
            pred_u_s1, pred_u_s2 = model(torch.cat((img_u_s1, img_u_s2))).chunk(2)

            # Tính pseudo-label và độ tin cậy cho unlabeled
            ...
            # === Tính loss cho unlabeled từ 2 phiên bản (sử dụng pseudo-label và độ tin cậy) ===
            ...
            # === Tính loss cho nhánh perturbed (pseudo-label của unlabeled) ===
